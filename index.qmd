...
listing:
    ...
    contents:
        ...
        - "content/chapter_3/index.qmd"
        - "content/chapter_4/index.qmd"
        - "content/chapter_5/index.qmd"
...
text_en = (
  "A step-by-step course to explore fundamentals of data science with {{< fa brands python >}}, "
...
# The function already creates <div class="list-chapter-ordered-en">
# So we wrap it in the parent div
final_html = (
...
```{ojs}
// Capture the content - need to get the PARENT div that contains language-specific divs
...
//| output: false
{
  const orderedDivs = document.querySelectorAll('.list-chapter-ordered-en, .list-chapter-ordered-fr');
  const unorderedDivs = document.querySelectorAll('.list-chapter-unordered');


  const lang = version_ui; // 'en'
  const otherLang =  'fr';
  ...
  if (ordre === 'vrac') {
    ...
    orderedDivs.forEach((div, i) => {
      ...
      // Now remove the other language version (French in your case)
      // The inner div has class "list-chapter-ordered-fr" or "list-chapter-ordered-en"
      const otherLangBlock = div.querySelector(`.list-chapter-ordered-${otherLang}`);
      if (otherLangBlock) {
        otherLangBlock.remove();
      }
    });
